...
frag_state_13_raw <- read_xls(here("Data", "New", "2_tab_state_13.xls"), range = cell_cols("B:C")) %>% 
  clean_names()

frag_state_18_raw <- read_xls(here("Data", "New", "2_tab_state_18.xls"), range = cell_cols("B:C")) %>% 
  clean_names()

# County level 2013 and 2018
frag_county_13_raw <- read_xls(here("Data", "New", "2_tab_county_13.xls"), range = cell_cols("B:Z"))

frag_county_18_raw <- read_xls(here("Data", "New", "2_tab_county_18.xls"), range = cell_cols("B:Z"))
...
#---------------------------------------
# 2013 state
frag_state_13 <- frag_state_13_raw %>% 
  rename(area_m = objec_864) %>% 
  mutate(area_acres = area_m * 0.000247105) %>% # Convert to acres
  mutate(year = "2013") %>% 
  select(value, year, area_acres)
  
# 2018 state
frag_state_18 <- frag_state_18_raw %>% 
  rename(area_m = objec_864) %>% 
  mutate(area_acres = area_m * 0.000247105) %>% # Convert to acres
  mutate(year = "2018") %>% 
  select(value, year, area_acres)
...
# Join years together
frag_state_full <- full_join(frag_state_13, frag_state_18) %>% 
...
# Plot
#---------------------------------------
fig_10 <- ggplot(data = frag_state_full, aes(x = frag_type, y = area_acres)) +
...
#---------------------------------------
# 2013 county
frag_county_13 <- frag_county_13_raw %>% 
  pivot_longer(!VALUE, names_to = "county", values_to = "area_m") %>% 
  rename(frag_type = VALUE) %>% 
  mutate(acres_2013 = area_m * 0.000247105) %>% # Convert to acres
  select(county, frag_type, acres_2013)
  
# 2018 county
frag_county_18 <- frag_county_18_raw %>% 
  pivot_longer(!VALUE, names_to = "county", values_to = "area_m") %>% 
  rename(frag_type = VALUE) %>% 
  mutate(acres_2018 = area_m * 0.000247105) %>% # Convert to acres
  select(county, frag_type, acres_2018)

# Join years together
frag_county_full <- full_join(frag_county_13, frag_county_18) %>% 
  mutate(county = str_replace_all(county, "_", " ")) %>% # Fix county name formatting
