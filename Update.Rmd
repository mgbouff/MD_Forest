...
state_change_compare <- state_change_acres %>% 
  ...
  mutate(acres_change = signs(acres_change, format = scales::number, plus_sign = TRUE)) %>% 
   mutate(year = case_when(
    year == "lu_2013_acres" ~ "2013",
    year == "lu_2018_acres" ~ "2018"))

ggplot(data = state_change_compare, aes(x = lu, y = tot_acres)) +
...
ggplot(data = state_change_compare, aes(x = lu, y = tot_acres)) +
  geom_bar(aes(alpha = year), fill = "#009b77",
           position="dodge", stat="identity") +
