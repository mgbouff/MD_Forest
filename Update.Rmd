...
fig_6 <- ggplot(data = plantable_area_pop, aes(x = county, y = plantable_ac)) +
  geom_col(aes(fill = county), color = "gray40") +
  labs(x = "Jurisdiction", 
       y = "Total Area (Acres)") +
  scale_fill_manual(values = county_pallette) +
  scale_y_continuous(labels=comma_format()) +
...
ggsave(here("Graphics", "fig6_plantable_area.jpg"), fig_6, height = 6, width = 7)
...
# Plot
#---------------------------------------
fig_35 <- ggplot(projected_lu_pop, aes(x = county, y = acres)) +
  geom_col(aes(fill = county)) +
  scale_fill_manual(values = county_pallette) +
