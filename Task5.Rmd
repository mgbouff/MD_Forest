...
lu_tc_pallette <- c("#F9DB6D", # Barren, light yellow
                 "#D9682A", # Buildings, brick
                 "#F08701", # Impervious, orange 
                 "#8DBE87", # Low veg, light green
                 "#F9A620", # Roads, light orange
...
pfa_acres <- pfa_data %>% 
  ...
  mutate(acres_change = as.numeric(acres_change))

# Plot
#---------------------------------------
pfa_acres_plot <- ggplot(data = pfa_acres, aes(x = type, y = acres_change)) +
  geom_col(aes(fill = type)) +
  labs(y = "Area Change (Acres)") +
...
```{r}
#---------------------------------------
# PFA percent change by county and land use
#---------------------------------------
