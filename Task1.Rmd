...
county_forest <- read.csv(here("Data", "1a_County_short_2018.csv")) %>% 
  clean_names() # Blank row added for proportion plots

# 1b.
# Read in Plantable Area data
plantable_county <- read.csv(here("Data", "1b_plantableareas_2018_county.csv")) %>% 
  clean_names() %>% 
...
plantable_county_plot <- ggplot(data = plantable_county, aes(x = fct_rev(county), y = area_acres)) +
  ...
  labs(x = "Jurisdiction", 
       y = "Total Area (Acres)",
       title = "Plantable Area by Jurisdction") +
  scale_fill_manual(values = county_pallette) +
...
plantable_percent_plot <- ggplot(data = plantable_county, 
                                 aes(x = fct_rev(county), y = p_area)) +
  geom_col(aes(fill = county)) +
  scale_fill_manual(values = county_pallette) +
  labs(title = "Percent Plantable Area by Jurisdiction",
       x = "Jurisdiction",
...
ggsave(here("Graphics", "plantable_percent.jpg"), plantable_percent_plot)
