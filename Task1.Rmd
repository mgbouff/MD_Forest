...
county_pop_stack <- county_forest %>% 
  ...
  mutate(type = fct_relevel(type, "Inside Forest", "Outside Forest")) %>% 
  mutate(county = as.factor(county)) %>% 
  mutate(county = fct_reorder(county, desc(x2020_pop)))

# Plot
#---------------------------------------
ggplot(data = county_pop_stack, aes(x = fct_rev(county), y = percent)) +
  geom_bar(stat = "identity", position = position_stack(reverse = TRUE), aes(fill = type)) + 
  scale_fill_manual(values = c("#24693d", "#6eb663")) +
  coord_flip() +
  theme_minimal()
